...
- Plan lives on the local 任务 page. Slot snapshots pin **all unfinished** tasks (schedule is not a gate). Auto-Core needs a matched mainline span ≥ 13 min; unmatched work goes to AI, never auto-Core from a work path alone. Repeat is complete-then-spawn. Optional OS banners: Settings 「任务通知」 (default off). `user_version` is **5** (adds `notes`; judgment snapshots still omit notes).
