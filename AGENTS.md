...
- Plan lives on the local 任务 page. Slot snapshots pin **all unfinished** tasks (schedule is not a gate). Auto-Core needs a matched mainline span ≥ 13 min; unmatched work goes to AI, never auto-Core from a work path alone. Repeat is complete-then-spawn. Optional OS banners: Settings 「任务通知」 (default off). With the Settings TickTick switch on, mapped lists import into the four preset groups and title / time / completion / deletion sync both ways; tokens live in `secrets.json`. `ticktick_cache` is not read and is not uploaded. Judgment still reads local unfinished tasks, not TickTick. `user_version` is **6** (adds TickTick link columns; judgment snapshots still omit notes).
