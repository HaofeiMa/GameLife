...
- **Plan is local.** Scheduling lives on the 任务 page (`task_lists` / `tasks`). Repeat is complete-then-spawn one next instance; uncomplete does not delete that row. Slot start pins unfinished tasks regardless of schedule. Observation still beats plan. **An empty board does not auto-core** — unmatched samples stay gray for text AI / vision (`judge.rs` no longer has `empty_quests_work_path_still_auto_cores`). The `ticktick_cache` table is kept empty; leftover TickTick secrets in `secrets.json` are not read.
...
- **UI is Chinese, and energy is called 能量** — never XP / 经验 in user-facing text, even though the ledger column stays `xp_delta`. No FullCalendar, no dnd-kit, no Radix, no self-reported timing. Task CRUD / NL input live on 任务; 今日’s plan column reuses the same right-click / drag / resize / copy, and changes day only via the date dialog.
...
- `2026-09-15-tasks-interaction-design.md` — task-board UX (alignment, context submenu, drag reorder, date dialog, calendar resize, activity ribbon, Today plan column, copy) plus match-based judgment. Slot snapshots pin **all unfinished** tasks; AI prompt cap 20; auto-Core is matched-mainline `strong_core` only; repeat is complete-then-spawn; `user_version` 4; Settings 「任务通知」.
