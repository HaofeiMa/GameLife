...
cargo test --offline -p gamelife          # DB, scheduler, sync, settle, macOS, TickTick, AI
...
- **Commands that touch the network must be `async`.** Tauri runs synchronous commands on the main thread, so a blocking HTTP call in one freezes the webview for its whole duration. The async commands are `ticktick_tree` (only when `refresh: true`), `ticktick_sync`, `ticktick_begin_oauth` / `ticktick_finish_oauth`, `test_vision_provider`, `sync_now_cmd`, `sync_test_connection`, `sync_list_devices`, and `sync_restore`. Opening 设置 must read cached status (`ticktick_tree` with `refresh` omitted/false, `sync_status`); only 同步任务 / 立即备份 / an explicit refresh hits the network.
...
- **Cloud backup is a copy, never a judgment input.** `VACUUM INTO` copies the whole live database; `SYNCED_TABLES` only drives reported counts. The copy must then delete `NEVER_SYNCED_TABLES` (`heartbeat`, `ticktick_cache`, `task_lists`, `tasks`) and trim `app_meta` to `device_id` alone. `secrets.json` and `screenshots/` never enter any payload, at any scope. `sync.enabled = false` ⇒ no outbound request from the sync module. Network failure must not block sampling, judgment, or settlement. Restore stages a file next to the live DB and never swaps it under the sampler.
...
- **Every page must keep working without TickTick, without cloud sync, and with the window closed.** Toasts are the only reward feedback (no system notifications, no sound); missing them is explicitly not a failure.
