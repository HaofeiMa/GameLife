...
- **`src-tauri`** — the app shell. `db.rs` (schema + `PRAGMA user_version` migrations, target `user_version = 6`; `device_id` is an additive column + `AFTER INSERT` trigger, not a version bump), `observe/` (platform seam: `imp` is `macos/` / `windows/` / `linux/`), `sampler.rs` (the 15s loop behind a `SampleSource` trait so tests inject a fake; every 20 ticks it may spawn a cloud sync and ping `task_notify::sync_now()`), `scheduler.rs` (~4k lines: slot lifecycle, random capture scheduling, policy seeding, gray-zone AI, finalize, midnight), `resolve.rs` (one transaction writing `JudgeOutput` into `slots` + daily rollups; ledger writes only in `Immediate` mode), `settle.rs` (day-level payment from the merged view when `Deferred`), `sync.rs` (WebDAV / S3 snapshot backup, `merged.db` rebuild, ownership / readiness), `commands.rs` (Tauri commands), `vision.rs` / `text_ai.rs` (HTTP to OpenAI-compatible providers), `task_notify.rs` (optional OS banners for scheduled tasks), `config.rs` (`config.json`, including `SyncSettings` and `task_notifications`), `keychain.rs` (`secrets.json`), `platform.rs` (the per-OS data dir and the default-browser opener — the only place a platform convention is decided).
...
- `notify.rs` — fire-and-forget `UNUserNotificationCenter` banners for scheduled tasks (`objc2` `msg_send`; no extra crate). Windows / Linux stubs are no-ops.

Data flow: sampler tick → `samples` row + `heartbeat`; `ensure_slot` pins the policy version, quest version, and **all unfinished** local tasks for the slot (`snapshots_open`; schedule is not a gate); `tick_capture` takes one screenshot at a random 4–13 min offset (persisted as `capture_scheduled_at`); at slot end `finalize_slot_end` → hints → `judge_slot` (core) → `resolve_slot` (slot + rollups; ledger only if `settlement_mode` is `Immediate`).
...
1. **Hard rules per sample** (`hint.rs`): locked/paused → `Away`; distraction rules → `Distraction`; idle ≥ 180s → `Away`; GameLife’s own window → `Side`; then **unfinished-task match** (all four roles, ignore `start`/`end`): same-role hit → mainline `CoreCandidate` / side-or-longterm `Side` / chore `Admin`; mixed or none → `Unsure`. Policy admin/side lists do not auto-classify after the hard rules, and a work path no longer mints `CoreCandidate` without a match. **There is no app whitelist** — `trusted_apps` does not gate candidacy. The idle gate sits between distraction and task match on purpose — a video left playing stays entertainment, while an unattended window becomes absence instead of matching. Observation beats plan — a slot whose samples hit an entertainment rule is entertainment and pays 0 even if an unfinished mainline title matches.
2. **Decidable from metadata** (`metadata_decidable`): matched-mainline `strong_core` ≥ 13 min auto-cores (not `grounded_strong_core`); dominant distraction auto-distracts — no AI call at all. Unmatched grounded work stays gray.
3. **Slot-end text AI** (`text_ai.rs`) for what remains gray: with a pinned local-task snapshot it does a task match (`select_prompt_snapshots` caps the prompt at 20; local matching uses the full pinned set), without one a category match against the four one-line `category_guides`.
...
- **Quests and policy are versioned per slot** (`quest_versions` / `policy_versions`): edits affect only slots that have not started. Local `task_lists` / `tasks` are the plan source: 任务 page edits them; slot start pins **all unfinished** tasks via `snapshots_open` (schedule is not a gate). The AI prompt is capped at 20 (`select_prompt_snapshots`); do not reject `upsert` at 20. `ticktick_cache` is not read. `user_version` is **6** (adds TickTick link columns; judgment snapshots still omit notes).
...
- **Plan is local.** Scheduling lives on the 任务 page (`task_lists` / `tasks`). Repeat is complete-then-spawn one next instance; uncomplete does not delete that row. Slot start pins unfinished tasks regardless of schedule. Observation still beats plan. **An empty board does not auto-core** — unmatched samples stay gray for text AI / vision (`judge.rs` no longer has `empty_quests_work_path_still_auto_cores`). With the Settings TickTick switch on, mapped lists import into the four preset groups and title / time / completion / deletion sync both ways; tokens live in `secrets.json`. `ticktick_cache` is not read and is not uploaded. Judgment still reads local unfinished tasks, not TickTick.
...
- **UI is Chinese, and energy is called 能量** — never XP / 经验 in user-facing text, even though the ledger column stays `xp_delta`. No FullCalendar, no dnd-kit, no Radix, no self-reported timing. Task CRUD / NL input live on 任务; 今日’s plan column reuses the same right-click / drag / resize / copy / `TaskDetailDialog`, and changes day only via the date dialog. Notes never enter judgment.
...
- **Every page must keep working with an empty task board, without cloud sync, and with the window closed.** Toasts are the only **reward** feedback (missing them is not a failure). Optional OS banners are task reminders (`config.task_notifications`, Settings 「任务通知」, default off) — request permission only when the switch turns on; a system denial is not a product failure.
...
- `2026-09-11-judgment-grounded-core-design.md` — supersedes the idle-based `away` rule and introduced `grounded_strong_core`. The idle-based `away` rule was later reinstated at 180s **after distraction and before task match** on the user's call; that spec's `reading_bridge` is now dead. Its “grounded path ≥ 13 min auto-Cores with no task match” is superseded by `2026-09-15-tasks-interaction-design.md`.
...
- `2026-09-13-activity-monitor-design.md`, `2026-09-13-analytics-design.md`, `2026-09-13-shop-desire-design.md` — judgment + shell / statistics / shop. TickTick-as-planner in the activity-monitor spec is superseded by `2026-09-15-local-tasks-design.md`. “No OS notifications” is superseded by the Settings 「任务通知」 toggle.
...
- `2026-09-15-local-tasks-design.md` — fifth rail tab 任务; local groups + NL input + drawn calendar replace TickTick as the plan source. Backup includes the task tables; `merged.db` does not merge them. Its timed-only snapshots, 20-item write cap, sort button, no-repeat/no-remind, no edge-resize, and “empty board still auto-Cores” rules are superseded by the next spec.
