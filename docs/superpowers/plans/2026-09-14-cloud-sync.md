...
- **Built and green:** T10 (tagging), T11 (`merged.db`), T12 (owner rule), T13 (readiness gate), T14a (deferred destination). T10 and T11 touch only the migration and the sync module; T12/T13 are pure. T14a adds a destination but no rule — with the mode left at `Immediate` the judgment path is byte-for-byte what it was.
- **Unblocked and next:** T14b (settler) and T14c (mode plumbing), then T15 (今日 showing 待结算预览). §13.1 was decided on 2026-09-14 — the user accepted that with two devices the 今日 page's 能量 becomes a 待结算预览.
- **Correction while preparing T14b (2026-09-14):** T14a's `pending_rewards` table was designed as "record the events, replay them later". That model is unsound for the *cumulative* keys, and §7.5 was rewritten (now §7.5.1 / §7.5.2). Nothing in T14a's code is wrong — `Deferred` simply won't be reached for cumulative keys once the settler exists — but the plan's T14b description and one of T14a's tests both rested on the replay model and are re-based here.
...
- [ ] **T14 Settlement** — split into three, because the risky part (touching the ledger) is much smaller than it first looked. Design recorded as spec **§7.5**.
  - [x] **T14a Deferred destination** — **done**. `SettlementMode { Immediate, Deferred }` stored in `app_meta['settle_mode']`; a new `pending_rewards` table with the *same* primary key as `ledger`; `resolve.rs` routes all six reward writes through one `record_reward` helper. The reward computation is untouched — deferral is a pure change of destination, which is exactly the property the tests state. `admin_xp_slots_today` counts whichever destination the mode writes to, so the four-a-day cap still behaves per device. Unrecognised or absent mode reads as `Immediate`: the failure direction is always "pay as usual". 5 tests, including a direct assertion that the two modes produce identical event sets — **that assertion is re-based in T14b**, because it encodes the replay model §7.5.1 rejects.
  - [ ] **T14b Settler** — for each day past `rewards_settled_through` where `day_is_ready`, take the owner-filtered slot sequence for that day from `merged.db` and walk it in `slot_start` order, accumulating `credited_core_seconds` / `credited_side_seconds` / `credited_chore_seconds`; call the **same** `gamelife-core` pure functions (`tick_keys_for_credited` / `tick_keys_for_discount`) the per-slot path calls, and pay the resulting events. Apply the day-level four-slot cap on `xp_admin` here (it is a day-level rule, so a day-level actor owns it). Advance `app_meta['rewards_settled_through']`.
    - **Not a replay of `pending_rewards`.** See spec §7.5.1: cumulative keys (`validated_coin:*`, `ladder:*`, …) are functions of the *day's* cumulative credited seconds, and each device's cumulative starts at its own 0. Two devices each spanning half a day emit the *same* keys, and the unique key silently swallows the second half — half a day worked, half paid, no error. §7.3's key uniqueness is the safety net; the normalised sequence is the source of correctness.
    - `slots.early_coins INTEGER` must be added and written by `upsert_slot` in both modes: `compute_early_coins` reads `samples`, and `aggregate` never uploads `samples`, so the merged view **cannot** rebuild that number.
    - `xp_support` / `xp_admin` need no stored intent — `category` and the credited seconds are both in the merged view.
    - **Re-base `deferring_records_exactly_what_the_ledger_would_have_paid`**, which currently asserts the two modes produce identical event sets. That is deliberately no longer true for cumulative keys; the test should assert the *local* events match and the cumulative ones are recomputed.
