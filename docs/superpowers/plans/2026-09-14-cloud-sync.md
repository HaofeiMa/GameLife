...
- [ ] **T14 Settlement** — split into three, because the risky part (touching the ledger) is much smaller than it first looked. Design recorded as spec **§7.5**.
  - [x] **T14a Mode switch** — **done, then narrowed in T14b**. `SettlementMode { Immediate, Deferred }` stored in `app_meta['settle_mode']`; `resolve.rs` routes its six reward writes through one `record_reward` helper; unrecognised or absent mode reads as `Immediate`, so the failure direction is always "pay as usual". T14a's `pending_rewards` holding pen was **deleted in T14b** — it encoded the per-device baseline §7.5.1 rejects, so it stored wrong numbers that nothing read. `Deferred` now writes no reward rows at all. 5 tests.
  - [x] **T14b Settler** — **done**. New `src-tauri/src/settle.rs`. `day_events(day, &[CanonicalSlot])` is the pure half: it walks the day in `slot_start` order accumulating core/side/chore seconds and calls the **same** `gamelife-core` functions `resolve_slot` calls, so the day's worth of rewards has one implementation and only its input changes. `read_canonical_day` opens `merged.db` **read-only** and returns `Ok(None)` for "no view" versus `Ok(Some(vec![]))` for "a day with nothing in it". `settle_due_days` walks from `app_meta['rewards_settled_through']` to yesterday, **stopping at the first day that is not ready** (a day that is not ready usually means a device is behind, and the days after it are missing that device too), pays each ready day **whole** — not just this device's slots, or a day whose owner was offline past grace would have those slots unpaid forever — and advances the watermark in the same transaction as the inserts.
    - `slots.early_coins INTEGER` added and written by `upsert_slot` in both modes: `compute_early_coins` reads `samples`, and `aggregate` never uploads `samples`, so the merged view **cannot** rebuild that number.
