...
这条规则管的是**合并视图里显示哪一份**，不是本机那份：既有的 `scheduler::settle_day`（本地午夜跑）早就把本机的 `days` 行写好了，而合并视图的 `days` 按 `(device_id, day)` 作键，两台设备的两行都留着。所以 §7.4 是给统计页用的取舍规则，不改变任何一台设备的本地记录。
...
1. `resolve_slot` 在 `Deferred` 模式下**照旧写槽行与日统计，但不写任何奖励行** —— 既不写 `ledger`，也不写任何「待定表」。奖励规则一行不动，改的只是「本机今天不落账」。
   - 不设待定表是 §7.5.1 的直接结论：本机算出的累计事件**不是**当日事件的一个子集，而是基于错误基准的另一组数字（两台设备各占半天会产出同一批键）。把那些数字记下来，等于把 bug 记下来。
2. **结算器（settler）**从 `merged.db` 的 owner 过滤后槽序列，按 `slot_start` 升序走一遍，累加 `credited_core_seconds` / `credited_side_seconds` / `credited_chore_seconds`，调用 `gamelife-core` 里**同一批纯函数**（`tick_keys_for_credited` / `tick_keys_for_discount`）产出当天的累计事件并落账。规则没有第二份实现，变的只是输入从「本机槽」换成「规范化槽序列」。
   - **整日一次性落账，不只落本机拥有的槽。** 水位线一天只前进一次，所以如果每台设备只落自己的槽，一个 owner 离线超过宽限期的日子，那些槽就**永远不会**被落账。整日落账是安全的：每台设备从同一份合并行推导出同一个序列，因而算出同样的键与金额，先跑的那台落账，其余的拿到 `AlreadyApplied`（§7.3）。归属过滤的作用是**让序列规范**（每个 `(day, slot_start)` 只留一行），不是分工。
...
6. 结算器只结算 `day_is_ready` 成立的日期：归属判定需要完整视图，不完整时算出的 owner 可能不对。宽限期过后照常结算 —— 迟到的设备数据只进统计视图，不改账本（§3.4 已显式接受）。此时合并视图可能仍缺该设备，`early_start` 的金额会取先落账那一方的值；这是宽限期换来的已知代价。
   - 遍历在**第一个不 ready 的日期停下**，不跳过它去结算后面的日子：某天不 ready 通常意味着有台设备落后，它后面的日子同样缺那台设备的数据，跳过就会拿不完整的视图去套归属规则 —— 正是 `day_is_ready` 要拦的事。宽限期保证了这不会变成永久阻塞。
   - 结算器**读不到合并视图就不结算**，且不推水位线。它的全部输入就是合并视图，所以调用顺序必须是「先重建合并视图，成功了再结算」。这也正是「判定时什么都不用记」成立的原因：没结算的日子一直没结算，水位线不动，奖励不会因为网络问题丢失。
   - 合并视图**只读打开**（§5.2 说它是派生数据）。
   - 「快照覆盖了 D」由 `devices.json` 的 `last_seen` 回答：它是远端唯一携带的时间戳，一台设备在 D 结束之后上传过，就是一台快照覆盖了 D 的设备。
   - 但覆盖度只统计**本次真的读到**的设备，不统计注册表声称存在的设备。两者恰好在「已登记但 `latest.db` 拉不到」时分叉 —— 那正是「拿缺一台设备的视图去套归属规则」的情形，也正是 `day_is_ready` 要拦的事。把注册表的 `last_seen` 直接透传会把没到的数据报成「已覆盖」。
7. 结算进度记在 `app_meta['rewards_settled_through']`（一个日期），由结算器读写。缺失时初始化为**昨天** —— 今天本来就还不能结算，而更早的日期已经由即时模式发过了。这个值必须**写下来**而不是每次由 `now` 推：一个跟着时钟走的默认值明天还会答「昨天」，它本该负责的那一天就永远结算不到。
8. 切换模式**不做历史回填**：已经在即时模式下发过的币不会重发（键唯一天然保证），也不会因为切到延迟模式而少发 —— 少发的只是切换之后新判定的槽，它们由结算器补上。
9. **调用顺序**（§8 的触发点共用一条路径）：上传 → 按刚取到的注册表写 `settle_mode` → 重建合并视图 → 结算。
   - 写模式**不是尽力而为**的一步：一台刚变成「第二台」的设备必须**先**停止逐槽发币，再开始结算，否则它会先发掉自己那半天，结算器随后把整天再发一遍。
   - 重建与结算的失败**只记录、不冒泡**：上传已经成功，把它报成整次同步失败会让设置页对一份已经落地的备份说「失败」。水位线只在真的落账时才动，所以下一个 tick 重试即可。
